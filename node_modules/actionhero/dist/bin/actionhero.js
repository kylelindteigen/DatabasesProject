...
(async () => {
    ...
    const handleUnbuiltProject = async (commands) => {
        try {
            // when generating the project from scratch, we cannot rely on the normal initializers
            const ExportedRunnerClasses = require(path.join(__dirname, "methods", commands.join(path.sep)));
            if (Object.keys(ExportedRunnerClasses).length > 1) {
                throw new Error("actionhero CLI files should only export one method");
            }
            const runner = new ExportedRunnerClasses[Object.keys(ExportedRunnerClasses)[0]]();
    ...
    const handleMethod = async (commands) => {
        console.log("₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋");
            ...
            if (!ExportedClasses) {
                for (const pluginName in config.plugins) {
                    if (config.plugins[pluginName].cli !== false) {
                        ...
                        p = path.join(pluginPath, "dist", "bin", commands.join(path.sep) + ".js");
                        if (fs.existsSync(p)) {
                            ExportedClasses = require(p);
                        }
                    }
    ...
    optimist.argv._.forEach(function (arg) {
        commands.push(arg);
    });
    const argsMatch = (a, b) => {
