...
const cli_1 = require("./../../classes/cli");
const projectRoot_1 = require("./../../classes/process/projectRoot");
const fileUtils_1 = require("../../modules/utils/fileUtils");
class Generate extends cli_1.CLI {
    constructor() {
        super();
        this.name = "generate";
        this.description =
    ...
    async run() {
        ...
        for (const name in oldFileMap) {
            ...
            if (extension === "ts" || extension === "js" || extension === "json") {
                ...
                documents[name] = documents[name].replace('from "./../index"', 'from "actionhero"');
            }
        }
        const AHversionNumber = JSON.parse(documents.packageJson).version;
        documents.packageJson = String(fs.readFileSync(path.join(__dirname, "/../../../templates/package.json.template")));
        documents.packageJson = documents.packageJson.replace("%%versionNumber%%", AHversionNumber);
        documents.readmeMd = String(fs.readFileSync(path.join(__dirname, "/../../../templates/README.md.template")));
        ...
        [
            ...
            try {
                const message = fileUtils_1.createDirSafely(projectRoot_1.projectRoot + dir);
                console.log(message);
            }
            catch (error) {
        ...
        for (const file in newFileMap) {
            try {
                const message = fileUtils_1.createFileSafely(projectRoot_1.projectRoot + file, documents[newFileMap[file]]);
                console.log(message);
            }
            catch (error) {
                console.log(error.toString());
