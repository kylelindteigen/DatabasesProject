...
class Help extends index_1.CLI {
    constructor() {
        super();
        this.name = "help";
    ...
    async run() {
        ...
        console.log();
        if (index_1.config.general.cliIncludeInternal !== false) {
            glob.sync(path.join(__dirname, "**", "**/*(*.js|*.ts)")).forEach((f) => {
                files.push(f);
            });
        }
        // CLI commands included in this project
        index_1.config.general.paths.cli.forEach((cliPath) => {
            glob.sync(path.join(cliPath, "**", "*(*.js|*.ts)")).forEach((f) => {
                files.push(f);
            });
        });
        // CLI commands from plugins
        Object.keys(index_1.config.plugins).forEach((pluginName) => {
            const plugin = index_1.config.plugins[pluginName];
            if (plugin.cli !== false) {
        ...
        files = utils_1.utils.arrayUnique(files);
        files.forEach((f) => {
            try {
