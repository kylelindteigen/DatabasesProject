...
class Exceptions extends index_1.Initializer {
    ...
    async initialize(config) {
        ...
        const consoleReporter = (error, type, name, objects, severity) => {
            ...
            if (type === "loader") {
                ...
                for (const i in relevantDetails) {
                    ...
                    if (objects.connection[relevantDetail] !== null &&
            ...
            for (const m in extraMessages) {
                index_1.log(extraMessages[m], severity);
            }
            let lines;
            try {
            ...
            for (const l in lines) {
                const line = lines[l];
                index_1.log("! " + line, severity);
            }
            index_1.log("*", severity);
        };
        ...
        index_1.api.exceptionHandlers.action = (error, data, next) => {
            let simpleName;
            try {
            ...
            index_1.api.exceptionHandlers.report(error, "action", name, { connection: data.connection }, "error");
