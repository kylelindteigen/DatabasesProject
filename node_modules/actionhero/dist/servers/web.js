...
const browser_fingerprint_1 = require("browser_fingerprint");
const index_1 = require("../index");
class WebServer extends index_1.Server {
    constructor() {
        super();
        this.type = "web";
        this.attributes = {
    ...
    async initialize() {
        ...
        this.fingerPrinter = new browser_fingerprint_1.BrowserFingerprint(this.config.fingerprintOptions);
    }
    async start() {
        let bootAttempts = 0;
        if (this.config.secure === false) {
            ...
            this.server = https.createServer(this.config.serverOptions, (req, res) => {
                this.handleRequest(req, res);
            });
        }
        ...
        if (this.config.enableEtag && fileStream && fileStream.path) {
            const filestats = await new Promise((resolve) => {
                fs.stat(fileStream.path, (error, filestats) => {
                    if (error || !filestats) {
                        this.log("Error receving file statistics: " + String(error), "error");
                    }
    ...
    handleRequest(req, res) {
        ...
        const cookies = index_1.utils.parseCookies(req);
        ...
        if (this.config.allowedRequestHosts &&
            this.config.allowedRequestHosts.length > 0) {
            let guess = "http://";
            if (this.config.secure) {
                guess = "https://";
            }
            const fullRequestHost = (req.headers["x-forwarded-proto"]
        ...
        const { ip, port } = index_1.utils.parseHeadersForClientAddress(req.headers);
    ...
    async completeResponse(data) {
        ...
        if (this.config.metadataOptions.serverInformation &&
            typeof data.response !== "string") {
            const stopTime = new Date().getTime();
            data.response.serverInformation = {
                serverName: index_1.config.general.serverName,
                apiVersion: index_1.config.general.apiVersion,
                requestDuration: stopTime - data.connection.connectedAt,
                currentTime: stopTime,
            };
        }
        if (this.config.metadataOptions.requesterInformation &&
            typeof data.response !== "string") {
            data.response.requesterInformation = this.buildRequesterInformation(data.connection);
        }
        if (data.response.error) {
            if (this.config.returnErrorCodes === true &&
        ...
        if (!data.response.error &&
            ...
            index_1.api.actions.actions[data.params.action][data.params.apiVersion]
        ...
        if (data.response.error) {
            data.response.error = await index_1.config.errors.serializers.servers.web(data.response.error);
        }
        let stringResponse = "";
    ...
    extractHeader(connection, match) {
        ...
        while (i >= 0) {
            if (connection.rawConnection.responseHeaders[i][0].toLowerCase() ===
                match.toLowerCase()) {
    ...
    respondToOptions(connection) {
        if (!this.config.httpHeaders["Access-Control-Allow-Methods"] &&
        ...
        if (!this.config.httpHeaders["Access-Control-Allow-Origin"] &&
    ...
    async determineRequestParams(connection) {
        ...
        if (pathParts[0] &&
            index_1.utils.arrayStartingMatch(urlPathForActionsParts, pathParts)) {
            requestMode = "api";
            ...
            index_1.utils.arrayStartingMatch(urlPathForFilesParts, pathParts)) {
            requestMode = "file";
        ...
        if (requestMode === "api") {
            ...
            if (connection.rawConnection.method !== "GET" &&
        ...
        if (requestMode === "file") {
            ...
            if (connection.params.file === "" ||
                ...
                connection.params.file =
                    connection.params.file + index_1.config.general.directoryFileType;
            }
            try {
    ...
    fillParamsFromWebRequest(connection, varsHash) {
        ...
        const collapsedVarsHash = index_1.utils.collapseObjectToArray(varsHash);
    ...
    buildRequesterInformation(connection) {
        ...
        for (const p in connection.params) {
            if (index_1.config.general.disableParamScrubbing === true ||
                index_1.api.params.postVariables.indexOf(p) >= 0) {
    ...
    cleanHeaders(connection) {
        ...
        for (const i in originalHeaders) {
            ...
            if (foundHeaders.indexOf(key.toLowerCase()) >= 0 &&
                key.toLowerCase().indexOf("set-cookie") < 0) {
                // ignore, it's a duplicate
            }
            else if (connection.rawConnection.method === "HEAD" &&
