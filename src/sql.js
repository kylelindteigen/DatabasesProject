...
class SQL{
	...
	static createRestUser(username, name, email, password, style, address){
		return new Promise(async (resolve,reject) =>{
			try{
				db.query("INSERT INTO User(Username, Name, Email, Password) VALUES(\""+username+"\", \""+name+"\", \""+email+"\", PASSWORD(\""+password+"\"))", function (err, result, fields) {
					if(err) throw err;
					db.query("INSERT INTO Restaurant(UserID, Style, Address) VALUES(\""+result.InsertedID +"\", \""+style+"\", \""+address+"\")", function (err2, result2, fields2) {
						if(err2) throw err2;
						resolve(result)
					});
				});
	...
	static createReview(userid, restid, description){
		return new Promise(async (resolve,reject) =>{
			try{
				db.query("INSERT INTO Review(UserCID, UserRID, ItemID, Description ) VALUES("+userid+", "+restid+", 0, \""+description+"\")", function (err, result, fields) {
					if(err) throw err;
					resolve(result);
				});
			}catch(e){
